...
def status(config: Path) -> None:
    ...
    async def _show_status():
        try:
            ...
            console.print(f"Memory Watchdog: {'[green]Active[/green]' if status.get('memory_watchdog_active') else '[red]Inactive[/red]'}")
            ...
            if status.get("resource_sharing"):
                ...
                if snapshots:
                    ...
                    for node in sorted(snapshots.keys()):
                        ...
                        swap_pct = s.get("memory", {}).get("swap_percent")
                        gpu_used = s.get("gpu", {}).get("used")
                        gpu_total = s.get("gpu", {}).get("total")
                        pressure = s.get("pressure", {}).get("overall", 0.0)

                        cpu_text = _fmt_percent(cpu_pct)
                        mem_text = _fmt_percent(mem_pct)
                        swap_text = _fmt_percent(swap_pct)
                        
                        if gpu_used is not None and gpu_total is not None:
                            gpu_text = f"{int(gpu_used)}/{int(gpu_total)}"
                        else:
                            gpu_text = "-"
                            
                        pressure_text = f"{pressure*100:.1f}%"
                        if pressure > 0.8:
                            pressure_text = f"[red]{pressure_text}[/red]"
                        elif pressure > 0.5:
                            pressure_text = f"[yellow]{pressure_text}[/yellow]"

                        util_table.add_row(str(node), cpu_text, mem_text, swap_text, gpu_text, pressure_text)
...
@resource_sharing.command()
@click.option("--node", "-n", default="gpu-master", help="Node ID to evacuate")
@click.option("--config", "-c", type=click.Path(exists=True, path_type=Path), default=Path("config/my-cluster-enhanced.yaml"))
def cooldown(node: str, config: Path) -> None:
    """Aggressively offload all processes from a node to clear load."""
    setup_logging()
    ...
    async def _cooldown():
        try:
            ...
            await orchestrator.initialize()
            console.print(f"[bold red]Triggering emergency cooldown for {node}...[/bold red]")
            count = await orchestrator.evacuate_node(node)
            console.print(f"[green]✓[/green] Evacuated {count} processes/objects from {node}")
...
@cli.group()
def daemon():
    """Continuous background monitoring and mitigation."""
    pass


@daemon.command()
@click.option("--config", "-c", type=click.Path(exists=True, path_type=Path), default=Path("config/my-cluster-enhanced.yaml"))
@click.option("--pid-file", default="/tmp/grid_orchestrator.pid", help="Path to PID file")
def start(config: Path, pid_file: str) -> None:
    ...
    async def _run_daemon():
        ...
        try:
            console.print(f"[green]Starting Grid Orchestrator Daemon (PID: {os.getpid()})[/green]")
