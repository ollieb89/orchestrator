...
class ResourceSnapshot:
    ...
    def memory_pressure(self) -> float:
        ...
        swap_penalty = (
            self.swap_percent / 100.0
        ) * 0.2  # Up to 20% increase based on swap
...
class ResourceMetricsCollector:
    ...
    def __init__(
        ...
        head_only_mode: bool = False,
    ):
        """Initialize the metrics collector.

        Args:
            cluster_config: The cluster configuration.
            ssh_manager: Optional SSH manager for remote metrics collection.
            collection_interval: Interval in seconds between metrics collection cycles.
            history_size: Maximum number of snapshots to keep in history per node.
            head_only_mode: When True, pressure events only fire for the head node.
        """
        ...
        # Head-only mode filtering
        self._head_only_mode = head_only_mode
        self._head_node_id = (
            cluster_config.nodes[0].name if cluster_config.nodes else None
        )
    ...
    def register_pressure_callback(
        self, callback: Callable[[str, ResourceType, float], Any]
    ) -> None:
    ...
    def set_head_only_mode(self, enabled: bool) -> None:
        ...
        When head-only mode is enabled, pressure events only fire for the head node.
        This is useful for auto-offload scenarios where we only want to monitor
        pressure on the master node.

        Args:
            enabled: True to enable head-only mode, False to monitor all nodes.
        """
    ...
    async def start(self) -> None:
        ...
        self._running = True
        logger.info(
            "Starting resource metrics collector", interval=self.collection_interval
        )
    ...
    async def _collect_all_metrics(self) -> None:
        ...
        for node_config in self.cluster_config.nodes:
            ...
            if len(self._resource_history[node_id]) > self.history_size:
                self._resource_history[node_id] = self._resource_history[node_id][
                    -self.history_size :
                ]

        # Check for pressure events after collecting all metrics
        self._check_pressure_events()

    def _trigger_pressure_event(
        self, node_id: str, resource_type: ResourceType, pressure: float
    ) -> None:
    ...
    def _check_pressure_events(self) -> None:
        """Check all nodes for pressure events and trigger callbacks.

        When head_only_mode is True, only checks pressure on the head node.
        This is used to prevent unnecessary offload triggers from worker nodes
        in auto-offload scenarios.
        """
        for node_id, snapshot in self._latest_snapshot.items():
            # Skip non-head nodes if head-only mode is enabled
            if self._head_only_mode and node_id != self._head_node_id:
                continue

            # Check memory pressure
            if snapshot.memory_pressure > self._memory_threshold_high:
                self._trigger_pressure_event(
                    node_id, ResourceType.MEMORY, snapshot.memory_pressure
                )

            # Check CPU pressure
            if snapshot.cpu_pressure > self._cpu_threshold_high:
                self._trigger_pressure_event(
                    node_id, ResourceType.CPU, snapshot.cpu_pressure
                )
    ...
    async def _collect_node_metrics(
        ...
        if (
            node_config.host == "localhost"
            or node_config.host == ray.util.get_node_ip_address()
        ):
    ...
    async def _collect_ssh_node_metrics(
        ...
        try:
            ...
            result = await self.ssh_manager.run_command(
                node_config.name, py_cmd, timeout=30
            )
            ...
            smi = await self.ssh_manager.run_command(
                node_config.name, smi_cmd, timeout=15
            )
            ...
            gpu_memory_available_bytes = max(
                0, int(gpu_memory_total_bytes) - int(gpu_memory_used_bytes)
            )
            ...
            logger.warning(
                "SSH metrics probe exception", node=node_config.name, error=str(e)
            )
            return None
    ...
    def _collect_ray_node_metrics(
        ...
        cpu_available = int(
            self._available_resources.get(f"node:{node_config.host}", 0).get("CPU", 0)
        )
        ...
        gpu_available = int(
            self._available_resources.get(f"node:{node_config.host}", 0).get("GPU", 0)
        )
        ...
        gpu_memory_used = (
            gpu_used * 8 * 1024 * 1024 * 1024
        )  # Estimate 8GB used per active GPU
        gpu_memory_available = gpu_memory_total - gpu_memory_used
    ...
    def _calculate_trend(
        self, snapshots: List[ResourceSnapshot], resource_type: ResourceType
    ) -> float:
